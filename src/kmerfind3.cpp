...
int main(int argc,char* argv[])
{
    ...
    const char *kmerfile="../data/test10M.dump";
    string fafile="../data/chm13.asm.p_ctg.compressed.fa";
    // FILE *fp = fopen("test3.pos","w");
    int t=1;
    ...
    while (getline(fa,line)){
        if(line[0] == '>'){
           //const char *name = line.substr(1,20).c_str();
		   name=line.substr(1,11);
        }
        else{
            ...
            for(uint32_t i = 21;i < len;i++){
                //for(int j=0;j<t;j++){
                    //auto start=clock();
                    if(kmer.find(&sbin)!=NULL){
                        //printf("%s\t%d\t%d\t%lu\t+\n",name,len,i-20,sbin);
                        // cout<<name<<"\t"<<len<<"\t"<<i<<"\t"<<sbin<<"\t+\n";
                        // break;
                        ;
                    }
                    else if(kmer.find(&rbin)!=NULL){
                        //printf("%s\t%d\t%d\t%lu\t-\n",name,len,i-20,rbin);
                        // cout<<name<<"\t"<<len<<"\t"<<i<<"\t"<<rbin<<"\t-\n";
						//break;
                        ;
                    }
                //}
